...
const url = `https://bdocodex.com/us/search/Morning Star`;

puppeteer.launch({headless:false, slowMo:300,args: ['--no-sandbox', '--disable-setuid-sandbox'] }) // Change headless to true so browser didn't launch. --no-sandbox argument for sudo users
  .then(browser => browser.newPage())
  .then(async page => {await page.goto(url);
    await page.select("select.form-control.input-sm","150"); // Drop down how many data to show (10,25,50,100,150,200)
  ...
  .then(html => {
    ...
    $('tbody>tr').each(function() {
      ...
      collect.push({id:id, name:name, link:link}); //Data Constructor
    });
    console.log(collect);
  });
